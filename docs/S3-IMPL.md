...
- F1 SPEC name recognizer is fragile: infer_spec_family requires exact glob *exchangeport*contract*spec* and fails on hyphenated exchange-port -> lost iterations. REQ: normalize basename before matching (strip hyphens/case) OR match on the Family: field inside the SPEC, not the filename.
