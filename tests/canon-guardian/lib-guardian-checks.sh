#!/usr/bin/env bash
# lib-guardian-checks.sh — single source of guardian checks (sourced by run.sh and self-check.sh).
# Defines run_guardian_checks(): operates on the current working directory, returns rc.
